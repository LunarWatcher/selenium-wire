...
class MitmProxyTest(TestCase):
    ...
    base_options_update = functools.partial(
        call, confdir="/some/dir", listen_host=host, listen_port=12345, ssl_insecure=True, anticomp=False
    )
    ...
    def test_disable_capture(self):
        proxy = MitmProxy(SeleniumWireOptions(self.host, 12345, disable_capture=True))
        self.assertEqual([], proxy.include_urls)
        self.assertEqual([".*"], proxy.exclude_urls)

    def test_shutdown(self):
        proxy = MitmProxy(SeleniumWireOptions(self.host, 12345))
